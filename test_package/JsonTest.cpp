...
int JsonTest() {
  ...
  {
    auto o = D(_name = "John"s, _age = 12, _children = {1, 2, 3, 4, 5},
               _city = D(_name = "Paris"s));
    ...
    assert(
        str ==
        R"json({"name":"John","age":12,"children":[1,2,3,4,5],"city":{"name":"Paris"}})json");
  ...
  {
    auto o =
        D(_name(_optional, _json_key = _my_json_name) = std::string("xxxx"));
...

  {
    typedef decltype(D(_name(_json_key = _username) = std::string(),
                       _age(_json_skip) = int(), _city = std::string())) User;
  ...
  {
    typedef decltype(D(_name(_json_key = _username) = std::string(),
                       _age = int(), _city = std::string())) user_scheme;
    ...
    std::cout << custom_user.name << " " << custom_user.age << " "
              << custom_user.city << std::endl;
  }
  ...
  // json_string :
  {
    ...
    std::string str =
        R"({"city": "major"})";
    json_decode(c, str);
	assert(c.city.str == R"("major")");

    str = R"({"city": "test"})";
    json_decode(c, str);
    std::cout << c.city.str << std::endl;
    assert(c.city.str == R"("test")");
  ...
  // Stringview
  {
    ...
    auto object =
        iod::D(_name = stringview(), _city = stringview(), _age = int());
  ...
  // Encode strings
  {
	...
	auto str = iod::json_encode(object);
	auto excpected = R"({"name":"\""})";
	std::cout << R"json(iod::D(_name = "\"") = )json" << str << "(expected: " << excpected << " )";
    assert(str == excpected);
  }
